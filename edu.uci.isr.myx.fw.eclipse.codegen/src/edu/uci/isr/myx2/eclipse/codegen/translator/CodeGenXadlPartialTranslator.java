...
public class CodeGenXadlPartialTranslator {
	...
	public CodeGenXadlPartialTranslator(XArchFlatInterface xarch, ObjRef xArchRef) {
		this.xarch = xarch;
	...
	public void translate(ObjRef brickRef, IMyxBrickExtension extBrick) {
		...
		XadlUtils.setXLinkByHref(xarch, brickRef, "type", MyxBrickExtensionUtils.getExtensionPointPluginUrl(extBrick).toString());
		...
		for (ObjRef interfaceEltRef : xarch.getAll(brickRef, "interface")) {
			...
			if (typeRef != null) {
				...
				if (extIntf != null) {
					...
					XadlUtils.setDirection(xarch, interfaceEltRef, CodeGenXadlPartialTranslator.getXadlDirection(extIntf.getDirection()));
		...
		for (IInterface intf : extIntfSet) {
			...
			XadlUtils.setDirection(xarch, interfaceEltRef, CodeGenXadlPartialTranslator.getXadlDirection(intf.getDirection()));
			...
			XadlUtils.setXLinkByHref(xarch, interfaceEltRef, "type", MyxBrickExtensionUtils.getExtensionPointPluginUrl(intf).toString());
		}
	}
