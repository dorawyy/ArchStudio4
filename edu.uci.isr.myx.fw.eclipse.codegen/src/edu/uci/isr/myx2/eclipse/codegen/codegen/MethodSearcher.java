...
import edu.uci.isr.myx2.eclipse.codegen.brick.MethodParameter;
import edu.uci.isr.myx2.eclipse.codegen.util.TextUtil;

/**
 * A methods searcher that gathers all the methods defined in the given
...
class MethodSearcher {
	 ...
	 */
	private List<MethodContainer> nonVoidMethodList = new ArrayList<MethodContainer>();

	/**
	 * methods whose return types are "void".
	 */
	private List<MethodContainer> voidMethodList = new ArrayList<MethodContainer>();

	/**
	 * constructors
	 */
	private List<MethodContainer> constructorList = new ArrayList<MethodContainer>();
	...
	/**
	 * Finds all the methods defined in the given interface and its ancestor
	 * interfaces.
	 * 
	...
	/**
	 * Finds methods defined in the specified interface and update the given lists according to 
	 * the return type of each method. This method does not look at the super types.
	 * 
	 * @param interfaceType
	 * @throws JavaModelException
	 */
	private static void findMethods(IType interfaceType, IMethodModifierChecker checker, List<MethodContainer> voidMethodList,
			List<MethodContainer> nonVoidMethodList) throws JavaModelException {
	 ...
	 * @throws JavaModelException
	 */
	private static MethodContainer createMethodContainer(IType type, IMethod method, String modifiers) throws JavaModelException {
		...
		for (int i = 0; i < paramTypes.length; i++) {
			...
			methodParams.add(new MethodParameter(paramType, paramName, TypeUtil.isPrimitive(paramTypes[i]), fqClassNames));
		}
	...
	private class AMethodChecker implements IMethodModifierChecker{
		 ...
		 * @param type
		 * @param method
		 * @return a string expression of modifiers the interfaceType must
		 *         implement, otherwise null
		 * @throws JavaModelException
		 */
		public String calcModifiers(IType type, IMethod method)
				throws JavaModelException {
	...
	/**
	 * Gets a collection of methods that are not implemented in the given type.
	 * TODO: experimenting
	 * @param type
	 * @param javaProject
	 * @return
	 * @throws JavaModelException
	 */
	public List<MethodContainer> findUnimplementedMethods(IType type, IJavaProject javaProject) throws JavaModelException {
