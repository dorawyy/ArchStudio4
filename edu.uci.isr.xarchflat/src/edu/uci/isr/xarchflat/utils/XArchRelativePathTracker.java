...
public final class XArchRelativePathTracker
	...
	public static class RequiredAttributeFilter
		...
		public boolean exclude(ObjRef objRef, ObjRef[] relativeAncestors, XArchFlatEvent evt, XArchPath relativeSourceTargetPath){
				...
				if(relativeSourceTargetPath.getLength() == 0){
					...
					if(objRef.equals(evt.getSource())){
						if(attributeName.equals(evt.getTargetName())){
							return !equalz(requiredValue, evt.getTarget());
						}
			...
			return !equalz(requiredValue, xarch.get(objRef, attributeName));
		}
	}

	public static class ProhibitedAttributeFilter
		implements IFilter{
		...
		public ProhibitedAttributeFilter(XArchFlatInterface xarch, String attributeName, String prohibitedValue){
			this.xarch = xarch;
			this.attributeName = attributeName;
		...
		public boolean exclude(ObjRef objRef, ObjRef[] relativeAncestors, XArchFlatEvent evt, XArchPath relativeSourceTargetPath){
			if(evt != null){
				if(relativeSourceTargetPath.getLength() == 0){
					...
					if(objRef.equals(evt.getSource())){
						if(attributeName.equals(evt.getTargetName())){
							return equalz(prohibitedValue, evt.getTarget());
						}
						// we are dealing with some other attribute
						return false;
					}
			...
			return equalz(prohibitedValue, xarch.get(objRef, attributeName));
		}
	}
	...
	public void stopScanning(){
		if(scanning){
			scanning = false;
			for(ObjRef toRemoveRef: addedObjRefToAncestorRefs.keySet().toArray(new ObjRef[addedObjRefToAncestorRefs.size()])){
				...
				if(rootObjRefIndex >= 0){
					...
					case XArchFlatEvent.REMOVE_EVENT: {
						if(relativePathMatchLength == relativePathNamesLength){
							...
							for(ObjRef toRemoveRef: addedObjRefToAncestorRefs.keySet().toArray(new ObjRef[addedObjRefToAncestorRefs.size()])){
								ObjRef[] ancestorRefs = addedObjRefToAncestorRefs.get(toRemoveRef);
								if(ancestorRefs != null && ancestorIndex < ancestorRefs.length){
