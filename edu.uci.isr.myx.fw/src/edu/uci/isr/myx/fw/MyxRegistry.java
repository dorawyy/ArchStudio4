...
public class MyxRegistry {
	...
	protected Map<IMyxBrick, ListenerListsOfType<Object>> brickToObjectMap = Collections.synchronizedMap(new HashMap<IMyxBrick, ListenerListsOfType<Object>>());
	...
	public synchronized void register(IMyxBrick b) {
		...
		brickToObjectMap.put(b, new ListenerListsOfType<Object>(Object.class));
	...
	public synchronized void map(IMyxBrick b, Object o) {
		ListenerListsOfType<Object> l = brickToObjectMap.get(b);
		if (l == null) {
	...
	public synchronized <T> T[] getObjects(IMyxBrick b, Class<T> ofType) {
		ListenerListsOfType<Object> l = brickToObjectMap.get(b);
		if (l == null) {
			// throw new IllegalArgumentException("No such brick registered: " +
			// MyxUtils.getName(b));
			return (T[]) Array.newInstance(ofType, 0);
		}
		return l.getListeners(ofType);
	}
	...
	public synchronized void fireMyxRegistryEvent(EventType eventType, IMyxBrick brick, Object object) {
		...
		for (MyxRegistryListener l : listeners.toArray(new MyxRegistryListener[listeners.size()])) {
