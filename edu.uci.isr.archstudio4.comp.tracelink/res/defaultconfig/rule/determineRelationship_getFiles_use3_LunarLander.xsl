...
<!-- This template goes through each top level object element and determines the files
     that are within a given group 
		 This has been customized for the Lunar Lander artifacts -->
	  ...
	  <xsl:choose>
	    ...
	    <xsl:variable name="recursiveResultOne">
			...
			<xsl:choose>
			  ...
			  <!-- 7/29/09 <xsl:when test="contains($curElement, '.htm') and not(contains($curElement, 'Chapter'))">  -->
			  <xsl:when test="contains($curElement, 'http')">  
			    <xsl:value-of select="concat('http ',$recursiveResultOne)"/>
			  </xsl:when>
			  <!-- 7/29/09 <xsl:when test="contains($curElement, '.htm') and contains($curElement, 'Chapter')">  -->
				<xsl:when test="contains($curElement, 'http') and contains($curElement, 'Chapter')">
			    <xsl:value-of select="concat('Chapter ',$recursiveResultOne)"/>
			  </xsl:when>
			  ...
			  <xsl:when test="contains($curElement, 'wiki')">
			    <xsl:value-of select="concat('wiki ',$recursiveResultOne)"/>
			  </xsl:when>
		   ...
		   <!-- next group number is different
			...
			<xsl:choose>
			  ...
			  <xsl:when test="contains($curElement, 'http') ">  
				...
				<!-- 7/29/09 <xsl:when test="contains($curElement, '.htm') and contains($curElement, 'Chapter')"> -->
			  <xsl:when test="contains($curElement, 'http') and contains($curElement, 'Chapter')">
			    <xsl:value-of select="string('Chapter ')"/>
			  </xsl:when>
			  <xsl:when test="contains($curElement, '.txt')">
			  ...
			  <xsl:when test="contains($curElement, 'wiki')">
			    <xsl:value-of select="string('wiki ')"/>
			  </xsl:when>
