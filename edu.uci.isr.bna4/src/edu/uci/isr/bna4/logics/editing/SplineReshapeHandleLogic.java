...
public class SplineReshapeHandleLogic extends AbstractThingLogic implements ISelectionTrackingListener, IBNAModelListener, IDragMoveListener,
        IStickyModeSpecifier {
		...
		if (m != null) {
			if (targetThing != null) {
				...
				editingMidpoints = targetThing instanceof IHasMutableMidpoints
				        && UserEditableUtils.isEditableForAnyQualities(targetThing, IHasMutableMidpoints.USER_MAY_ADD_MIDPOINTS,
				                IHasMutableMidpoints.USER_MAY_REMOVE_MIDPOINTS, IHasMutableMidpoints.USER_MAY_MOVE_MIDPOINTS);
				editingEndpoint1 = sd.includesEndpoint1 && targetThing instanceof IHasMutableEndpoints
				        && UserEditableUtils.isEditableForAnyQualities(targetThing, IHasMutableEndpoints.USER_MAY_MOVE_ENDPOINT1);
				editingEndpoint2 = sd.includesEndpoint2 && targetThing instanceof IHasMutableEndpoints
				        && UserEditableUtils.isEditableForAnyQualities(targetThing, IHasMutableEndpoints.USER_MAY_MOVE_ENDPOINT2);
				restickingEndpoint1 = editingEndpoint1
				        && UserEditableUtils.isEditableForAnyQualities(targetThing, IHasMutableEndpoints.USER_MAY_RESTICK_ENDPOINT1);
				restickingEndpoint2 = editingEndpoint2
				        && UserEditableUtils.isEditableForAnyQualities(targetThing, IHasMutableEndpoints.USER_MAY_RESTICK_ENDPOINT2);
	...
	protected synchronized void checkHandles() {
		...
		if (selectedThings.length == 1 && !equalz(targetThing, selectedThings[0])) {
			...
			if (selectedThing instanceof IHasMutableMidpoints
			        && UserEditableUtils.isEditableForAnyQualities(selectedThing, IHasMutableMidpoints.USER_MAY_ADD_MIDPOINTS,
			                IHasMutableMidpoints.USER_MAY_REMOVE_MIDPOINTS, IHasMutableMidpoints.USER_MAY_MOVE_MIDPOINTS)
			        || selectedThing instanceof IHasMutableEndpoints
			        && UserEditableUtils.isEditableForAnyQualities(selectedThing, IHasMutableEndpoints.USER_MAY_MOVE_ENDPOINT1,
	...
	public StickyMode[] getAllowableStickyModes(IThing thing, String propertyName, IThing stickToThing) {
		if (!thing.equals(stickToThing) && UserEditableUtils.isEditable(stickToThing)) {
			if (stickyModeSpecifiers.size() == 0) {
				if ("glass".equals(AssemblyUtils.getPartName(stickToThing)) && !(stickToThing instanceof ReshapeHandleGlassThing)
	...
	private void stickEndpointNear(IBNAView view, IBNAModel model, IThing targetThing, String propertyName, Point checkForStickyPoint, Point point,
	        boolean restick) {
			...
			if (movedThing instanceof ReshapeHandleGlassThing) {
				...
				if (rhtTargetThingID != null && rhtTargetThingID.equals(targetThing.getID())) {
					...
					if (pointNum == 0 && editingEndpoint1) {
						stickEndpointNear(evt.getView(), getBNAModel(), targetThing, IHasEndpoints.ENDPOINT_1_PROPERTY_NAME, evt.getWorldPoint(), evt
						        .getAdjustedWorldPoint(), restickingEndpoint1);
					}
					else if (pointNum == reshapeHandles.getAllReshapeHandleAssemblies().length - 1 && editingEndpoint2) {
						stickEndpointNear(evt.getView(), getBNAModel(), targetThing, IHasEndpoints.ENDPOINT_2_PROPERTY_NAME, evt.getWorldPoint(), evt
						        .getAdjustedWorldPoint(), restickingEndpoint2);
					}
