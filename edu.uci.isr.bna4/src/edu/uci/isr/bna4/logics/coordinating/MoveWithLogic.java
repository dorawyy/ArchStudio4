...
public class MoveWithLogic extends AbstractMaintainThingsWithReferenceNameLogic<IThing, IRelativeMovable> {
	...
	public MoveWithLogic(ReferenceTrackingLogic rtl) {
		super(null, new String[] { IHasBoundingBox.BOUNDING_BOX_PROPERTY_NAME, IHasAnchorPoint.ANCHOR_POINT_PROPERTY_NAME }, IRelativeMovable.class,
		        new String[] { MOVES_WITH_MODE_PROPERTY_NAME }, rtl, MOVES_WITH_THING_ID_PROPERTY_NAME);
	}
				...
				case TRACK_BOUNDING_BOX_FIRST:
					if (sourceThing instanceof IHasBoundingBox) {
						if (IHasBoundingBox.BOUNDING_BOX_PROPERTY_NAME.equals(thingEvent.getPropertyName())) {
							maintainWithBoundingBoxDelta(sourceModel, sourceThing, (Rectangle) thingEvent.getOldPropertyValue(), (Rectangle) thingEvent
							        .getNewPropertyValue(), targetThing);
						}
						break;
					}
					// fall through
				case TRACK_ANCHOR_POINT_ONLY:
					if (sourceThing instanceof IHasAnchorPoint) {
						if (IHasAnchorPoint.ANCHOR_POINT_PROPERTY_NAME.equals(thingEvent.getPropertyName())) {
							maintainWithAnchorPointDelta(sourceModel, sourceThing, (Point) thingEvent.getOldPropertyValue(), (Point) thingEvent
							        .getNewPropertyValue(), targetThing);
						}
					}
					break;
	...
	protected void maintainWithBoundingBoxDelta(IBNAModel model, IThing sourceThing, Rectangle oldBoundingBox, Rectangle newBoundingBox,
	        IRelativeMovable targetThing) {
		Lock lock = targetThing.getPropertyLock();
		lock.lock();
		try {
	...
	protected boolean isSourceThing(IBNAModel sourceModel, IThing sourceThing, ThingEvent sourceThingEvent) {
		return (sourceThing instanceof IHasAnchorPoint || sourceThing instanceof IHasBoundingBox)
		        && super.isSourceThing(sourceModel, sourceThing, sourceThingEvent);
	}
