...
public class BNAUtils {
	...
	public static Point worldToLocal(ICoordinateMapper cm, Rectangle worldRectangle, int worldCornerWidth, int worldCornerHeight) {
		Rectangle lc = new Rectangle(worldRectangle.x, worldRectangle.y, Math.min(worldCornerWidth, worldRectangle.width), Math.min(worldCornerHeight,
	...
	public static final void convertWorldToLocal(ICoordinateMapper cm, int[] xyArray) {
		for (int i = 0, length = xyArray.length; i < length;) {
			xyArray[i] = cm.worldXtoLocalX(xyArray[i++]);
			xyArray[i] = cm.worldYtoLocalY(xyArray[i++]);
		}
	}
	...
	public static void drawMarquee(Graphics g, ResourceUtils res, int offset, boolean reverse, Runnable r) {
		...
		g.setForegroundColor(res.getColor(null, SWT.COLOR_WHITE));
		r.run();
	...
	public static int[] toXYArray(Point[] points) {
		int[] xyArray = new int[2 * points.length];
		for (int i = 0, length = points.length, xy = 0; i < length; i++) {
			xyArray[xy++] = points[i].x;
			xyArray[xy++] = points[i].y;
		}
		return xyArray;
	}

	public static void renderWorld(IBNAView view, final Graphics g, Rectangle clip, ResourceUtils res) {
		Graphics og = g;
		g.pushState();
		try {
			for (IThing thing : view.getWorld().getBNAModel().getAllThings()) {
				if (!Boolean.TRUE.equals(thing.getProperty(IBNAView.HIDE_THING_PROPERTY_NAME))) {
					g.restoreState();
					boolean obscured = Boolean.TRUE.equals(thing.getProperty(IBNAView.OBSCURE_THING_PROPERTY_NAME));
					if (obscured) {
						if (og == g && !(og instanceof ObscuredGraphics)) {
							og = new ObscuredGraphics(g);
						}
						g.setAlpha(50);
					...
					peer.draw(view, obscured ? og : g, clip, res);
				}
			}
		}
		finally {
