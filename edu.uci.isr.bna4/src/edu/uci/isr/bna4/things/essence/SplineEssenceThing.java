...
public abstract class SplineEssenceThing extends RelativeMovableEssenceThing implements IHasMutableEndpoints, IHasMutableMidpoints, IHasBoundingBox, IHasPoints {

	public SplineEssenceThing(String id) {
		super(id);

		final Set<Object> pointPropertyNames = new HashSet<Object>();
		pointPropertyNames.add(IHasEndpoints.ENDPOINT_1_PROPERTY_NAME);
		pointPropertyNames.add(IHasEndpoints.ENDPOINT_2_PROPERTY_NAME);
		pointPropertyNames.add(IHasMidpoints.MIDPOINTS_PROPERTY_NAME);

		addThingListener(new IThingListener() {

			public void thingChanged(ThingEvent thingEvent) {
				if (pointPropertyNames.contains(thingEvent.getPropertyName())) {
					setProperty(IHasBoundingBox.BOUNDING_BOX_PROPERTY_NAME, calculateBoundingBox());
					setProperty(IHasPoints.POINTS_PROPERTY_NAME, calculatePoints());
				}
			}
		});
	}
	...
	public Point[] calculatePoints() {
		...
		if (mps != null && mps.length > 0) {
			allPoints.addAll(Arrays.asList(mps));
		}
	...
	public Point[] getPoints() {
		return getProperty(IHasPoints.POINTS_PROPERTY_NAME);
	}
