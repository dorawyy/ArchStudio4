...
public class BoxReshapeHandlesAssembly extends AbstractAssembly {
	...
	public BoxReshapeHandlesAssembly(IBNAModel model, IThing parentThing, Object assemblyKind) {
		...
		for (Orientation o : EnumSet.allOf(Orientation.class)) {
			if (!o.equals(Orientation.NONE)) {
				ReshapeHandleAssembly rh = new ReshapeHandleAssembly(model, parentThing, null);
	...
	public void remove(boolean andChildren) {
		...
		for(ReshapeHandleAssembly rh : getAllReshapeHandleAssemblies()){
			rh.remove(andChildren);
		}
	    super.remove(andChildren);
	}
