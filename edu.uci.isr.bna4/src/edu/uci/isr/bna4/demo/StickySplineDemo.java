	...
	static void populateModel(IBNAModel m) {
		...
		for (StickyMode stickyMode : StickyMode.values()) {
			PolygonAssembly p = new PolygonAssembly(m, null, stickyMode, new IPolygonGenerator() {
				...
				public Point[] calculatePoints(Rectangle r) {
					...
					int x2 = x1 + w;
					int y2 = y1 + h;
					return new Point[] { new Point(x1 + w / 2, y1), new Point(x2, y1 + h / 2), new Point(x1 + w / 2, y2), new Point(x1, y1 + h / 2),
					        new Point(x1 + w / 2, y1) };
				}
			});
			...
			UserEditableUtils.addEditableQuality(p.getPolygonGlassThing(), IHasMutableSelected.USER_MAY_SELECT, IRelativeMovable.USER_MAY_MOVE,
		...
		for (StickyMode fromStickyMode : StickyMode.values()) {
			for (StickyMode toStickyMode : StickyMode.values()) {
				...
				UserEditableUtils.addEditableQuality(s.getSplineGlassThing(), IHasMutableSelected.USER_MAY_SELECT, IRelativeMovable.USER_MAY_MOVE,
				        IHasMutableEndpoints.USER_MAY_MOVE_ENDPOINT1, IHasMutableEndpoints.USER_MAY_MOVE_ENDPOINT2,
				        IHasMutableMidpoints.USER_MAY_ADD_MIDPOINTS, IHasMutableMidpoints.USER_MAY_MOVE_MIDPOINTS,
				        IHasMutableMidpoints.USER_MAY_REMOVE_MIDPOINTS);
				UserEditableUtils.addEditableQuality(s.getSplineGlassThing(), IHasMutableEndpoints.USER_MAY_RESTICK_ENDPOINT1,
