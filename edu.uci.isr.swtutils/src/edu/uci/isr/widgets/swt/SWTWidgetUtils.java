...
public class SWTWidgetUtils {
	...
	public static void async(final Display d, final Widget w, final Runnable r) {
		 ...
		 * never awaken. This makes the system appear as to suddenly stop
		 * responding. The fix is to always wake the display thread after
	...
	public static void sync(final Display d, final Widget w, final Runnable r) {
		 ...
		 * For some reason, the display will occasionally fall asleep and is
		 * never awaken. This makes the system appear as to suddenly stop
		 * responding. The fix is to always wake the display thread before
		 * submitting something to run. NB: This must be before the syncExec
		 * command
		 */
		d.wake();
